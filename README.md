...
- интерфейс:
  - загрузка ВР (+ валидация его)
  - список всех ВР (вместе с информацией - имя, время загрузки, длина, размер)
  - удаление ВР


- подгрузка models в БД при старте

- обновить структуру проекта
- рассказать что должно лежать в .env

- celery + redis - вся эта тема с очередями задач для анализа и предсказания
- модель данных для задач - обновить
